...
export type WeiboImage = { url: string; noteId?: string; title?: string; sourceUrl?: string; author?: string; uid?: string };
...
// 按关键词搜索微博配图（m.weibo.cn 搜索 API，免登录）。走子进程 weibo-search.mts：
// chromium segfault 只杀子进程不拖垮主服务。keyword 用 base64 走 argv（中文安全）。
export async function searchWeiboByKeyword(keyword: string, limit = 15): Promise<WeiboImage[]> {
  const scriptPath = join(process.cwd(), 'src', 'scripts', 'weibo-search.mts');
  const keywordB64 = Buffer.from(keyword).toString('base64');
  return new Promise((resolve) => {
    execFile(process.execPath, ['--import', 'tsx', scriptPath, keywordB64, String(limit)], {
      maxBuffer: 50 * 1024 * 1024, timeout: 90000, windowsHide: true,
    }, (err: any, stdout: any) => {
      if (err) { console.error(`[weibo] 子进程搜索 "${keyword}" 失败: ${err.message}`); resolve([]); return; }
      try { resolve((JSON.parse(stdout).images || []) as WeiboImage[]); }
      catch { resolve([]); }
    });
  });
}
