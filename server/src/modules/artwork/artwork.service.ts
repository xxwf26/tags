...
import { eq, and, inArray, like, isNotNull, isNull } from 'drizzle-orm';
...
export class ArtworkService {
  // 多维筛选：tags=tagId,tagId（跨维度与、同维度或）；orient；kw；artistId；默认排除软删
  async list(query: Record<string, string>) {
    const { tags, orient, kw, artistId, sort } = query;
    const conds: any[] = [isNull(schema.artworks.deletedAt)];
  ...
  async create(data: {
    ...
    await logOperation({ type: 'artwork_create', targetType: 'artwork', targetId: artId, summary: `录入作品「${data.title || '未命名'}」` });
    return duplicateOf ? { ...out, duplicateOf } : out;
  }

  // 软删作品（可撤销）：标记 deleted_at，保留图文件与记录，写操作日志
  async remove(id: number) {
    const [row] = await db.select().from(schema.artworks).where(eq(schema.artworks.id, id));
    if (!row) throw new Error('作品不存在');
    await db.update(schema.artworks).set({ deletedAt: new Date() }).where(eq(schema.artworks.id, id));
    await logOperation({
      type: 'artwork_delete', targetType: 'artwork', targetId: id,
      summary: `删除作品「${row.title || '未命名'}」(#${id})`,
      payload: { artworkId: id, title: row.title, artistId: row.artistId }, undoable: true,
    });
    return { id, deleted: true, undoable: true };
  }

  // 恢复软删作品
  async restore(id: number) {
    await db.update(schema.artworks).set({ deletedAt: null }).where(eq(schema.artworks.id, id));
