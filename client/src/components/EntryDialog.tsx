      ...
      <div className="bg-white rounded-2xl max-w-lg w-full p-5 my-8" onClick={e => e.stopPropagation()}>
        ...
        <div className="space-y-3 text-sm">
          <div className="relative">
            <label className="text-xs text-stone-400">画师 <span className="text-stone-300">（输入名字搜索已有；新名字自动建画师）</span></label>
            <input value={artistName} onChange={e => { setArtistName(e.target.value); setArtistOpen(true); }}
              onFocus={() => setArtistOpen(true)} onBlur={() => setTimeout(() => setArtistOpen(false), 150)}
              placeholder="输入或选择画师名"
              className="w-full mt-1 border border-stone-200 rounded-lg px-3 py-2 focus:border-xhs outline-none" />
            ...
            {isNewArtist && <div className="text-[11px] text-amber-600 mt-1">新画师「{artistName.trim()}」将自动创建</div>}
          </div>
          <div>
